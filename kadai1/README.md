...
-   Java言語は**オブジェクト**指向型の言語であり,
    クラスを組み合わせてプログラムを作りあげる.
    クラスは**オブジェクト**の属性(データ)や機能(動作)を定義したものである.
-   クラス定義の中で**オブジェクト**の持つ属性(データ)を定義したものを
    **フィールド**とよび,機能(動作)を定義したものを**メソッド**とよぶ.
-   プログラムコードの中でnewを使って,クラスの**インスタンス**を生成することができる.
-   変数に格納できるもの（**フィールド**の引数の型に指定できるもの）は,
    intやdoubleなどの**基本**型と,**インスタンス**のメモリ中の場所を表わす**参照**型のどちらかである.
